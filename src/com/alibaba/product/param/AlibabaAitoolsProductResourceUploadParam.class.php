...
class AlibabaAitoolsProductResourceUploadParam {

        
        /**
    * @return 【使用精美版以外的版本时，此参数必填】商品主图数据，最大2MB，建议500*500像素以上，支持jpg/jpeg/png格式，白底图或其他纯色背景图片效果最佳；商品应占据图片主体位置，主体的在图片中占比建议大于80%；ISV 还应提示用户注意图片的方向，引导用户将旋转过的图片调整为自上而下的方向后，才能正常使用后续的识别能力，得到更加准确的识别结果；ISV 可以引导用户对图片进行美化后再上传；所有图片数据可以使用 base64 编码传输，也可以通过 HTTP Multipart Form 将文件直接提交到服务器（文件参数名与 API 参数名相对应）。
    */
    ...
    /**
     * 设置【使用精美版以外的版本时，此参数必填】商品主图数据，最大2MB，建议500*500像素以上，支持jpg/jpeg/png格式，白底图或其他纯色背景图片效果最佳；商品应占据图片主体位置，主体的在图片中占比建议大于80%；ISV 还应提示用户注意图片的方向，引导用户将旋转过的图片调整为自上而下的方向后，才能正常使用后续的识别能力，得到更加准确的识别结果；ISV 可以引导用户对图片进行美化后再上传；所有图片数据可以使用 base64 编码传输，也可以通过 HTTP Multipart Form 将文件直接提交到服务器（文件参数名与 API 参数名相对应）。     
    ...
    */
    ...
    public function setBarcodeBytes( $barcodeBytes) {
        ...
        /**
    * @return 条形码数字字符串，与 barcodeBytes 参数二选一。
    */
        public function getBarcodeText() {
        $tempResult = $this->sdkStdResult["barcodeText"];
        return $tempResult;
    }
    
    /**
     * 设置条形码数字字符串，与 barcodeBytes 参数二选一。     
     * @param String $barcodeText     
     * 参数示例：<pre>691234567890</pre>     
     * 此参数必填     */
    public function setBarcodeText( $barcodeText) {
        $this->sdkStdResult["barcodeText"] = $barcodeText;
    }
    
        
        /**
    * @return 【使用精美版时，此参数必填】用户已上传好的 3D 模型识别号，对应获取 3D 模型列表 API 出参的 product3DModels.id 字段（alibaba.product.ProductThreeDimModel 结构体的 id 字段）。使用精美版时会忽略 imageData 参数，商品主图将使用 3D 模型的白底主图。
    */
        public function getModel3DRecogniseID() {
        $tempResult = $this->sdkStdResult["model3DRecogniseID"];
        return $tempResult;
    }
    
    /**
     * 设置【使用精美版时，此参数必填】用户已上传好的 3D 模型识别号，对应获取 3D 模型列表 API 出参的 product3DModels.id 字段（alibaba.product.ProductThreeDimModel 结构体的 id 字段）。使用精美版时会忽略 imageData 参数，商品主图将使用 3D 模型的白底主图。     
     * @param String $model3DRecogniseID     
     * 参数示例：<pre>xxxxxxxxxxx-123</pre>     
     * 此参数必填     */
    public function setModel3DRecogniseID( $model3DRecogniseID) {
        $this->sdkStdResult["model3DRecogniseID"] = $model3DRecogniseID;
    }
